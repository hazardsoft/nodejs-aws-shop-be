...
const middleware = async (req: ServerRequest, res: ServerResponse, next: Next): Promise<void> => {
  ...
  const destinationServiceKey = envKeys.find((key) => originalPathname.startsWith(key))
  if (!destinationServiceKey) {
    respond(HTTP_STATUS.BAD_GATEWAY, { message: `Service (${originalPathname}) not found` }, res)
    return
  }
  ...
  const destinationPathname = originalUrl.pathname.substring(destinationServiceKey.length + 1)
